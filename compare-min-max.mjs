...
async function compareMinMax() {
    console.log("--- COMPARING CONDITIONS: Price > Weekly Max VS Price > Weekly Min ---");
    const now = new Date();
    ...
    for (const t of candidates) {
        ...
        // Process Case A: Price > Max
        k15m.forEach((k, idx) => {
            if (k.time < startOfDay || idx === 0) return;
            const fresh = k.close > wMax && k15m[idx - 1].close <= wMax;
            if (fresh && k.close > prevWeekVwap && k.close > currentWeekVwap) {
                let maxH = -Infinity;
        ...
        // Process Case B: Price > Min (Earlier entry)
        k15m.forEach((k, idx) => {
            if (k.time < startOfDay || idx === 0) return;
            const fresh = k.close > wMin && k15m[idx - 1].close <= wMin;
            if (fresh && k.close > prevWeekVwap && k.close > currentWeekVwap) {
                let maxH = -Infinity;
                for (let j = idx; j < k15m.length; j++) if (k15m[j].high > maxH) maxH = k15m[j].high;
                statsMin.total++;
