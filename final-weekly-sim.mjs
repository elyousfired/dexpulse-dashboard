...
async function finalSim() {
    ...
    for (const time of intervals) {
        ...
        for (const [symbol, data] of coinDataMap) {
            ...
            let wMax = -Infinity;
            data.k1d.forEach((dk, idx) => { if (getMonTs(dk.time) === monTs && dk.time < dayStart) { if (data.dailyVwaps[idx] > wMax) wMax = data.dailyVwaps[idx]; } });
    ...
    console.log(JSON.stringify({ tradeHistory, finalBalance }));
}
finalSim();
