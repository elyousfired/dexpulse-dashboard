...
async function runTurbo10USDSimulation() {
    console.log("--- v7+ TURBO $10 PER TRADE SIMULATION (FEB 26) ---");
    console.log("Settings: 1 Trade/Token | Trailing Stop (Act: 3%, Trail: 2%) | SL: 5%");
            ...
            if (cBull && cMax && cFresh && cTurbo && cAboveDVwap) {
                ...
                for (let j = idx + 1; j < k15m.length; j++) {
                    ...
                    if (trailed) {
                        ...
                        outcome = 'STOP LOSS';
                        profitUSD = -INVEST * (SL / 100);
                        break;
                    }
                }
                trades.push({ symbol: t.symbol, time: k.time, entry, outcome, profitUSD });
            }
        });
    }

    console.log(`\nResults ($10 per Trade):\n`);
    let net = 0;
    trades.forEach(tr => {
        console.log(`| ${new Date(tr.time).toISOString().slice(11, 16)} | ${tr.symbol} | ${tr.outcome} | $${tr.profitUSD.toFixed(2)} |`);
        net += tr.profitUSD;
    });

    console.log(`\n--- FINAL SUMMARY ---`);
    console.log(`Trades: ${trades.length}`);
    console.log(`Net Profit Today: $${net.toFixed(2)}`);
}

runTurbo10USDSimulation();
