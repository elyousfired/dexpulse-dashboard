...
const CONFIG = {
    // File paths - adjust if necessary for your VPS structure
    configFile: path.join(__dirname, 'server', 'bot_config.json'),
    huntsFile:  path.join(__dirname, 'server', 'data', 'active_hunts.json'),
    historyFile: path.join(__dirname, 'server', 'data', 'trades_history.json'),
...
/**
 * Symbol Picker: Top 200 by Quote Volume
 */
...
async function runHybridScanner() {
    ...
    try {
        ...
        for (const symbol of symbols) {
            ...
            const newHunt = { 
                symbol, entryPrice: v.last, entryTime: new Date().toISOString(), 
                peakPrice: v.last, currentPrice: v.last, status: 'active', strategyId: 'vwap_hybrid', 
                mode, density: dens, capital, tier: 1, rsi: Math.round(rsi) 
            };
            ...
            fresh.push(newHunt);
            if (saveHunts(fresh)) {
                log('Entry', mode === 'Sniper' ? '🎯' : '🚀', `${mode.toUpperCase()} ENTRY: ${symbol} at $${v.last}`);
                await sendTelegram(`${mode === 'Sniper' ? '🎯' : '🚀'} <b>${mode.toUpperCase()} ENTRY: #${symbol}</b>\nPrice: $${v.last}\nDensity: ${dens}%\nCapital: $${capital}`);
...
async function runHybridTracker() {
    ...
    try {
        ...
        for (const hunt of active) {
            try {
                const res = await axios.get(`https://api.binance.com/api/v3/klines?symbol=${hunt.symbol}&interval=1m&limit=2`);
                if (!res.data[1]) continue;
                const live = parseFloat(res.data[1][4]);
                hunt.currentPrice = live;
                if (live > hunt.peakPrice) hunt.peakPrice = live;

                const pnl = ((live - hunt.entryPrice) / hunt.entryPrice) * 100;
                
                // Track for Basket Exit (Stagnant 1h, +/- 1%)
                const ageMs = now - new Date(hunt.entryTime).getTime();
                if (ageMs > 60 * 60 * 1000 && pnl >= -1.0 && pnl <= 1.0) {
                    basket.push(hunt);
                    basketProfitUSD += hunt.capital * (pnl / 100);
                }
                ...
                // 2) LOCK PROFIT
                    ...
                    stopPrice = hunt.entryPrice * 1.01; 
                }
                ...
                if (live <= stopPrice) {
                    ...
                    const profit = hunt.capital * (pnl / 100);
                    config.totalBalance = (config.totalBalance || 120.81) + profit;
                    fs.writeFileSync(CONFIG.configFile, JSON.stringify(config, null, 2));
                    
                    saveToHistory(hunt); // LOG TO PERMANENT HISTORY
                    
                    log('Exit', '💸', `CLOSED ${hunt.symbol} | PnL: ${pnl.toFixed(2)}% | Net: $${profit.toFixed(2)}`);
        ...
        const feesUSD = basket.length * CONFIG.fixedSlotCap * 0.003; 
        if (basket.length >= 3 && basketProfitUSD > feesUSD) {
            ...
            for (const bh of basket) {
                ...
                if (target) {
                    ...
                    const profit = target.capital * (((target.currentPrice - target.entryPrice) / target.entryPrice));
                    config.totalBalance = (config.totalBalance || 120.81) + profit;
                    saveToHistory(target); // LOG TO PERMANENT HISTORY
                }
            }
            fs.writeFileSync(CONFIG.configFile, JSON.stringify(config, null, 2));
