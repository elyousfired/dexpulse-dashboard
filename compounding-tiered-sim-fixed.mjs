...
async function runWeeklyCompoundingSimulationFixed() {
        ...
        for (const [symbol, data] of coinDataMap) {
            ...
            if (k.close > data.prevWeekVwap && k.close > cVW && k.close > wMax && prevK.close <= wMax && cVW > data.prevWeekVwap && volatility > 0.02) {
                activeTrades.push({
                    ...
                    entry: k.close,
                    peak: k.close,
                    timeEntry: new Date(time).toISOString(),
    ...
    tradeHistory.sort((a, b) => a.timeEntry.localeCompare(b.timeEntry));
    ...
    console.log(`\n================================`);
    console.log(`INITIAL CAPITAL : $200.00`);
    console.log(`FINAL BALANCE  : $${finalBalance.toFixed(2)}`);
    console.log(`TOTAL PROFIT    : $${(finalBalance - 200).toFixed(2)} (${(((finalBalance - 200) / 200) * 100).toFixed(2)}%)`);
    console.log(`================================`);
}
runWeeklyCompoundingSimulationFixed();
