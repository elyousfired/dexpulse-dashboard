...
async function runWeeklyCompoundingSimulationFinalAudit() {
    ...
    for (const time of intervals) {
        ...
        activeTrades = activeTrades.filter(t => {
            ...
            if (k.low <= slPrice) {
                ...
                tradeHistory.push({ ...t, exit: slPrice, pnl: -5, endTime: new Date(time).toISOString(), reason: 'SL', finalVal });
                return false;
            }
            if (profitPct >= 0.03 && k.low <= exitPrice) {
                ...
                slotWallets[t.walletIdx] = finalVal;
                tradeHistory.push({ ...t, exit: exitPrice, pnl: finalPnl, endTime: new Date(time).toISOString(), reason: 'Trailing', finalVal });
                return false;
            }
            return true;
        });
        ...
        for (const [symbol, data] of coinDataMap) {
            ...
            if (tradeHistory.find(h => h.symbol === symbol && new Date(h.timeEntry).setUTCHours(0, 0, 0, 0) === dayStart)) {
                continue;
            }
            ...
            if (isV7) {
                if (symbol === 'DENTUSDT') console.log(`DENT MATCH! Time: ${new Date(time).toISOString()} | WalletIdx: ${freeWalletIdx}`);
                activeTrades.push({
                    symbol, walletIdx: freeWalletIdx, entry: k.close, peak: k.close, timeEntry: new Date(time).toISOString(), initialCapital: slotWallets[freeWalletIdx]
                });
            }
        }
    }
    ...
    console.log(`PROFIT          : $${(finalBalance - 200).toFixed(2)}`);
    console.log(`================================`);
}
runWeeklyCompoundingSimulationFinalAudit();
