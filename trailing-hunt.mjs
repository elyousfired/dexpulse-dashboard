...
async function runTrailingSimulation() {
    console.log("--- v7+ TURBO TRAILING STOP SIMULATION (FEB 26) ---");
    console.log("Strategy: Trailing Stop | Hard SL: 5% | Activation: 2.5% Profit | Trail: 2% from High");

    const INVEST_PER_TRADE = 100;
    const HARD_SL = 5;
    const TRAIL_ACTIVATION = 2.5;
        ...
        k15m.forEach((k, idx) => {
            ...
            if (cTrend && cAboveMax && cFresh && cTurbo && cVol && cAboveDVwap) {
                ...
                for (let j = idx + 1; j < k15m.length; j++) {
                    ...
                    if (high > maxPriceSeen) maxPriceSeen = high;

                    const pnlHigh = ((maxPriceSeen - entry) / entry) * 100;
                    const pnlLow = ((low - entry) / entry) * 100;

                    // Activate Trailing
                    if (pnlHigh >= TRAIL_ACTIVATION) trailActive = true;

                    // Exit Conditions
                    if (trailActive) {
                        const dropFromHigh = ((maxPriceSeen - close) / maxPriceSeen) * 100;
                        if (dropFromHigh >= TRAIL_DISTANCE) {
                            outcome = 'TRAILED EXIT';
                            profitUSD = INVEST_PER_TRADE * ((close - entry) / entry);
                            break;
                        }
                    }
                    ...
                    if (pnlLow <= -HARD_SL) {
                        outcome = 'HARD STOP';
                        profitUSD = -INVEST_PER_TRADE * (HARD_SL / 100);
                        break;
                    }
                }
                ...
                trades.push({
                    ...
                    outcome,
                    profitUSD,
                    maxGain: ((maxPriceSeen - entry) / entry) * 100
                });
            }
        });
    }

    console.log(`\nSimulation Results ($100/Trade, Trailing Stop):\n`);
    let totalCash = 0;
    trades.forEach(tr => {
        console.log(`| ${tr.time.slice(11, 16)} | ${tr.symbol} | Max: +${tr.maxGain.toFixed(2)}% | Outcome: ${tr.outcome} | Profit: $${tr.profitUSD.toFixed(2)} |`);
        totalCash += tr.profitUSD;
    });

    console.log(`\nSUMMARY:`);
    console.log(`Total Trades: ${trades.length}`);
    console.log(`Success (Trailed/TP): ${trades.filter(t => t.profitUSD > 0).length}`);
    console.log(`Fail (SL): ${trades.filter(t => t.profitUSD < 0).length}`);
    console.log(`Open: ${trades.filter(t => t.outcome === 'OPEN').length}`);
    console.log(`Final Net: $${totalCash.toFixed(2)}`);
}

runTrailingSimulation();
