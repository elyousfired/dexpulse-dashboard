...
export const TokenChart: React.FC<TokenChartProps> = ({
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            const intervalLimits: Record<string, number> = {
                '1m': 1000, '5m': 1000, '15m': 1000, '1h': 400, '4h': 100, '1d': 30
            };
            const fetchLimit = intervalLimits[interval] || 1000;

            try {
                const [data, weekly, dailyKlines] = await Promise.all([
                    fetchBinanceKlines(symbol, interval, fetchLimit),
                ...
                if (dailyKlines.length >= 2) {
                    ...
                    setTmaState({
                        metrics, classification, zones, liquidityTaken: detectionResult.liquidityTaken,
                        current: { ...detectionResult, bias, confidence: Math.max(probabilities.reversal, probabilities.continuation, probabilities.range) },
                        probabilities
                    });
                    ...
                    if (weekly) {
                        ...
                        const vwapDetection = analyzeVwapIntraday(vwapMetrics, today15m);
                        const vwapProbs = runVwapScenarioEngine(vwapMetrics, vwapDetection, today15m);
                        ...
                        setVwapArchState({ metrics: vwapMetrics, trend: vwapTrend, liquidityTaken: vwapDetection.liquidityTaken, current: { ...vwapDetection, bias: vwapBias, confidence: Math.max(vwapProbs.reversal, vwapProbs.continuation, vwapProbs.range) }, probabilities: vwapProbs });
                    }

                    if (candlestickSeriesRef.current && today15m.length > 0) {
                        const markers: any[] = [];
                        today15m.forEach((k) => {
                            if (k.high > metrics.pdh && k.close < metrics.pdh) markers.push({ time: k.time as any, position: 'aboveBar', color: '#ef4444', shape: 'arrowDown', text: '🔥 PDH SWEEP' });
                            if (k.low < metrics.pdl && k.close > metrics.pdl) markers.push({ time: k.time as any, position: 'belowBar', color: '#22c55e', shape: 'arrowUp', text: '🧲 PDL SWEEP' });
                        });
                        if (detectionResult.mss) markers.push({ time: data[data.length - 1].time as any, position: detectionResult.mss === 'Long' ? 'belowBar' : 'aboveBar', color: '#a855f7', shape: 'circle', text: `MSS ${detectionResult.mss === 'Long' ? 'UP' : 'DOWN'}` });
                    ...
                    if (showTma && pdhSeriesRef.current && pdlSeriesRef.current) {
                        ...
                        pdhSeriesRef.current?.setData([]); pdlSeriesRef.current?.setData([]); pdoSeriesRef.current?.setData([]); pdcSeriesRef.current?.setData([]); midLineSeriesRef.current?.setData([]); buyZoneSeriesRef.current?.setData([]); sellZoneSeriesRef.current?.setData([]);
                    }
                }
                ...
                if (chartRef.current && candlestickSeriesRef.current) {
                    ...
                    if (volumeSeriesRef.current) volumeSeriesRef.current.setData(!isFlowMode && showVolume ? data.map(d => ({ time: d.time as any, value: d.volume, color: d.close >= d.open ? 'rgba(34, 197, 94, 0.4)' : 'rgba(239, 68, 68, 0.4)' })) : []);

                    if (vwapSeriesRef.current) {
                        if (!isFlowMode && showVwap) {
                            const SLOPE_LOOKBACK = 10, ATR_LENGTH = 14, NEAR_ZERO_THRESHOLD = 0.05;
                            ...
                            const rawVwap = data.map(d => {
                                ...
                                lastDay = day; qVol += d.quoteVolume || (d.close * d.volume); bVol += d.volume;
                                return { time: d.time, value: bVol > 0 ? qVol / bVol : d.close };
                            });
                            const trueRanges = data.map((d, i) => { if (i === 0) return d.high - d.low; const prevClose = data[i - 1].close; return Math.max(d.high - d.low, Math.abs(d.high - prevClose), Math.abs(d.low - prevClose)); });
                            const atrValues = trueRanges.map((_, i) => { if (i < ATR_LENGTH - 1) return 0; const slice = trueRanges.slice(i - ATR_LENGTH + 1, i + 1); return slice.reduce((s, v) => s + v, 0) / ATR_LENGTH; });
                            const vValues = rawVwap.map((v, i) => {
                                let color = '#3b82f6';
                                if (i >= SLOPE_LOOKBACK && atrValues[i] > 0) {
                                    const slope = v.value - rawVwap[i - SLOPE_LOOKBACK].value;
                                    const dev = slope / atrValues[i];
                                    if (dev > NEAR_ZERO_THRESHOLD) color = '#facc15';
                                    else if (dev < -NEAR_ZERO_THRESHOLD) color = '#f43f5e';
                                }
                    ...
                    if (volumeCurveSeriesRef.current) {
                        if (!isFlowMode && showVolumeCurve) volumeCurveSeriesRef.current.setData(data.map((d, i, arr) => { const slice = arr.slice(Math.max(0, i - 19), i + 1); const avg = slice.reduce((s, c) => s + c.volume, 0) / slice.length; return { time: d.time as any, value: avg }; }));
                    ...
                    if (weeklyMaxSeriesRef.current && weeklyMinSeriesRef.current) {
                        if (!isFlowMode && showWeekly && weekly) {
                            ...
                            const fibRatios = [2.0, 1.618, 1.272, 0.786, 0.618, 0.5, 0.382, 0.236, -0.272, -0.618, -1.0];
                            vwapFibRefs.current.forEach((series, i) => { const level = weekly.min + wRange * fibRatios[i]; series.setData(data.map(d => ({ time: d.time as any, value: level }))); });
                            ...
                            if (last && weekly) {
                                ...
                                const prevV = weekly.prevWeekVwap || 0, currV = weekly.currentWeekVwap || 0;
                                const isGolden = last.close > weekly.max && last.close > currV && last.close > prevV && volatility > 0.02 && currV > prevV;
                            ...
                            weeklyMaxSeriesRef.current?.setData([]); weeklyMinSeriesRef.current?.setData([]); vwapFibRefs.current.forEach(s => s.setData([]));
                    ...
                    if (weeklyVwapCurveSeriesRef.current) {
                        if (!isFlowMode && showWeeklyVwap) {
                            const now = new Date(); const dayOfWeek = now.getUTCDay(); const diffToThisMon = dayOfWeek === 0 ? 6 : dayOfWeek - 1;
                            const thisMon = new Date(now); thisMon.setUTCHours(0, 0, 0, 0); thisMon.setUTCDate(now.getUTCDate() - diffToThisMon);
                            const prevMonTs = Math.floor(thisMon.getTime() / 1000) - (7 * 24 * 3600);
                            const filteredData = data.filter(d => d.time >= prevMonTs);
                            const weeklySeries = calculateWeeklyVwapSeries(filteredData);
                            weeklyVwapCurveSeriesRef.current.setData(weeklySeries.map(p => ({ time: p.time as any, value: p.value })));
                        } else weeklyVwapCurveSeriesRef.current.setData([]);
                    }
                }
                chartRef.current?.timeScale().fitContent();
            } catch (err: any) { console.error('[ChartEngine] Error:', err); setError(err.message); } finally { setLoading(false); }
        };
        fetchData();
        ...
        const cleanup = subscribeToKlines(address, interval, (kline) => {
            ...
            if (!isFlowMode) {
                ...
                if ((showWeeklyVwap || isVwapWeeklyView) && vwapData) {
                    let bg = '#0d0f14';
                    const rangeDist = Math.abs(vwapData.max - vwapData.min), volatility = rangeDist / kline.close;
                    const prevV = vwapData.prevWeekVwap || 0, currV = vwapData.currentWeekVwap || 0;
                    const isGolden = kline.close > vwapData.max && kline.close > currV && kline.close > prevV && volatility > 0.02 && currV > prevV;
                    if (isGolden) { bg = 'rgba(16, 185, 129, 0.08)'; candleColor = '#10b981'; }
                    else if (kline.close > vwapData.mid) { bg = 'rgba(234, 179, 8, 0.08)'; candleColor = '#facc15'; }
                    else if (kline.close > vwapData.min) { bg = 'rgba(59, 130, 246, 0.08)'; candleColor = '#3b82f6'; }
                    else { bg = 'rgba(239, 68, 68, 0.08)'; candleColor = '#f43f5e'; }
                    chartRef.current.applyOptions({ layout: { background: { type: ColorType.Solid, color: bg } } });
                }
                candlestickSeriesRef.current.update({ time: kline.time as any, open: kline.open, high: kline.high, low: kline.low, close: kline.close, color: candleColor, wickColor: candleColor });
                if (showVolume && volumeSeriesRef.current) volumeSeriesRef.current.update({ time: kline.time as any, value: kline.volume, color: kline.close >= kline.open ? 'rgba(34, 197, 94, 0.3)' : 'rgba(239, 68, 68, 0.3)' });
                if (showVwap && vwapSeriesRef.current) {
                    const day = new Date(kline.time * 1000).getUTCDate();
                    if (cumulativeRef.current.lastDay !== -1 && day !== cumulativeRef.current.lastDay) cumulativeRef.current = { quote: 0, base: 0, lastDay: day };
                    const qInc = kline.quoteVolume || (kline.close * kline.volume), bInc = kline.volume;
                    const liveVwap = (cumulativeRef.current.base + bInc) > 0 ? (cumulativeRef.current.quote + qInc) / (cumulativeRef.current.base + bInc) : kline.close;
                    vwapSeriesRef.current.update({ time: kline.time as any, value: liveVwap });
                    if (kline.isFinal) { cumulativeRef.current.quote += qInc; cumulativeRef.current.base += bInc; }
                }
            }
        });
        return () => cleanup();
    ...
    const IndicatorToggle = ({ active, onClick, icon: Icon, label, color }: any) => (
        <button onClick={onClick} className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg border transition-all duration-200 ${active ? `${color} border-current bg-current/10 font-bold shadow-lg shadow-current/5` : 'border-gray-800 text-gray-500 hover:text-gray-300 hover:bg-gray-800/50'}`}>
            <Icon className="w-3.5 h-3.5" />
            <span className="text-[10px] uppercase tracking-wider">{label}</span>
            {active ? <Eye className="w-3 h-3 ml-0.5 text-blue-400" /> : <EyeOff className="w-3 h-3 ml-0.5 opacity-50 text-gray-400" />}
        </button>
    );
        ...
        <div className="flex flex-col h-full bg-[#0d0f14] rounded-2xl overflow-hidden border border-gray-800 shadow-2xl">
            <div className="flex items-center justify-between p-4 border-b border-gray-800 bg-gray-900/20 backdrop-blur-md">
                <div className="flex items-center gap-4">
                    <div className="flex flex-col"><span className="text-white font-black text-sm tracking-tighter uppercase">{symbol}</span><div className="flex items-center gap-2 mt-0.5"><span className="px-1.5 py-0.5 rounded bg-blue-500/10 text-blue-400 text-[9px] font-black border border-blue-500/20">{interval}</span>{vwapData && (<div className="flex flex-col gap-0.5 ml-1"><span className="text-[9px] text-gray-500 font-bold leading-none">MAX: <span className="text-emerald-500">${vwapData.max.toFixed(4)}</span></span><span className="text-[9px] text-gray-500 font-bold leading-none">MIN: <span className="text-rose-500">${vwapData.min.toFixed(4)}</span></span></div>)}</div></div>
                ...
                <div className="flex bg-black/60 rounded-xl p-1 border border-gray-800">
                    {INTERVALS.map((int) => (<button key={int.value} onClick={() => setInterval(int.value)} className={`px-3 py-1.5 text-[10px] font-black rounded-lg transition-all ${interval === int.value ? 'bg-blue-600 text-white shadow-lg shadow-blue-500/20' : 'text-gray-500 hover:text-gray-300'}`}>{int.label}</button>))}
                </div>
            </div>
            <div className="flex-1 flex flex-wrap xl:flex-nowrap overflow-hidden group">
                <div className="flex-1 relative">
                    {loading && (<div className="absolute inset-0 z-20 flex items-center justify-center bg-[#0d0f14]/80 backdrop-blur-sm"><div className="flex flex-col items-center gap-3"><RefreshCcw className="w-8 h-8 text-blue-500 animate-spin" /><span className="text-[10px] font-black text-gray-500 animate-pulse">SYNCING DATA...</span></div></div>)}
                    <div ref={chartContainerRef} className="w-full h-full" />
                    {showTma && tmaState && (
                        <div className="absolute top-4 left-1/2 -translate-x-1/2 z-10 flex flex-col items-center gap-2 pointer-events-none">
                            <div className={`px-4 py-1.5 rounded-full border shadow-2xl backdrop-blur-md flex items-center gap-3 ${tmaState.current.state === 'ACCEPT ABOVE' || tmaState.current.state === 'ACCEPT BELOW' ? 'bg-red-500/20 border-red-500/40' : tmaState.current.state === 'SWEEPING LIQUIDITY' ? 'bg-amber-500/20 border-amber-500/40' : 'bg-blue-500/20 border-blue-500/40'}`}><div className={`w-2 h-2 rounded-full animate-pulse ${tmaState.current.state === 'ACCEPT ABOVE' || tmaState.current.state === 'ACCEPT BELOW' ? 'bg-red-500' : tmaState.current.state === 'SWEEPING LIQUIDITY' ? 'bg-amber-500' : 'bg-blue-500'}`} /><span className="text-[11px] font-black text-white tracking-[0.2em] uppercase">{tmaState.current.state}</span></div>
                            {tmaState.current.mss && (<div className="px-3 py-1 rounded-lg bg-purple-500/20 border border-purple-500/40 flex items-center gap-2"><Zap className="w-3 h-3 text-purple-400" /><span className="text-[9px] font-black text-purple-400 tracking-wider">STRUCTURE SHIFT: {tmaState.current.mss}</span></div>)}
                        </div>
                    )}
                </div>
                {showTma && (<div className="w-full xl:w-[380px] xl:border-l border-t xl:border-t-0 border-gray-800 bg-[#06080c]/50 backdrop-blur-xl overflow-y-auto p-6 max-h-[600px] xl:max-h-none"><TmaPanel symbol={symbol} state={tmaState} isLoading={loading} /></div>)}
            </div>
            <div className="flex items-center justify-between px-5 py-2.5 border-t border-gray-800 bg-gray-900/40">
                <div className="flex items-center gap-6"><div className="flex items-center gap-2"><Activity className="w-3.5 h-3.5 text-blue-500" /><span className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Market Feed</span></div>{vwapData && (<div className="flex items-center gap-4 text-[10px] font-bold"><span className="text-gray-500">WEEKLY FLOOR: <span className="text-rose-500">${vwapData.min.toFixed(4)}</span></span><span className="text-gray-500">DAILY PIVOT: <span className="text-amber-500">${vwapData.mid.toFixed(4)}</span></span></div>)}</div>
