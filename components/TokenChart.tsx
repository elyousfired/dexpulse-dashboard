...
export const TokenChart: React.FC<TokenChartProps> = ({
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            // Calculate limit to cover ~2 weeks (15 days)
            ...
            if (chartRef.current && candlestickSeriesRef.current) {
                ...
                if (weeklyMaxSeriesRef.current && weeklyMinSeriesRef.current) {
                    ...
                    // 6. Continuous Weekly VWAP Curve (Limited to Last Week + This Week)
                    if (weeklyVwapCurveSeriesRef.current) {
                        if (!isFlowMode && showWeeklyVwap) {
                            // Filter data to only include from previous Monday onwards
                            const now = new Date();
                            const dayOfWeek = now.getUTCDay(); // 0 = Sun
                            const diffToThisMon = dayOfWeek === 0 ? 6 : dayOfWeek - 1;
                            const thisMon = new Date(now);
                            thisMon.setUTCHours(0, 0, 0, 0);
                            thisMon.setUTCDate(now.getUTCDate() - diffToThisMon);
