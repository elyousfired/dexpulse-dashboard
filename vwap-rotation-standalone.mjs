...
/**
 * ═══════════════════════════════════════════════════════════════════════════════
 *  🛰️  VWAP ROTATION — STANDALONE AUTONOMOUS BOT
 * ═══════════════════════════════════════════════════════════════════════════════
 *
 ...
 *    ✅ Balance Tracking
 *
 *  Run:   node vwap-rotation-standalone.mjs
 *  Stop:  Ctrl+C
 *  PM2:   pm2 start vwap-rotation-standalone.mjs --name vwap-rotation
 *
 * ═══════════════════════════════════════════════════════════════════════════════
 */
...
const CONFIG = {
    ...
    // Rotation Slots
    maxSlots: 3,                     // Maximum concurrent positions
    ...
    // Exit: Stop Loss
    hardStopPct:       0.04,         // -4% hard stop loss
    ...
    // Exit: Hyper-Scalp (Quick Profit Lock)
    scalpTriggerPct:   0.025,        // +2.5% peak triggers scalp mode
    scalpReversalPct:  0.010,        // 1.0% reversal from peak → exit

    // Exit: Moonshot TP
    moonshotPct:       0.20,         // +20% instant take profit
...
/**
 * Calculate Anchored VWAP from entry time to now
 * Used for AVWAP Trend Guard — exit when AVWAP slope turns negative
 *
 * Anchored VWAP = Σ(TypicalPrice × Volume) / Σ(Volume) from entry time
 *   where TypicalPrice = (High + Low + Close) / 3
 */
async function calculateAnchoredVwap(symbol, entryTimeMs) {
    const url = `https://api.binance.com/api/v3/klines?symbol=${symbol}&interval=15m&startTime=${entryTimeMs}&limit=1000`;
    const { data: klines } = await axios.get(url, { timeout: 10000 });
    ...
    return {
        avwap: totalV > 0 ? totalPV / totalV : 0,
...
async function closeHunt(hunt, exitPrice, reason) {
    ...
    await sendTelegram([
        `${hunt.pnl >= 0 ? '🟢' : '🔴'} <b>VWAP ROTATION EXIT: #${hunt.symbol}</b>`,
        ``,
...
async function checkExitConditions(hunt) {
    ...
    let stopPrice = hunt.entryPrice * (1 - CONFIG.hardStopPct);  // -4%
    let trailDist = 0.05;
    let newTier = 1;

    // Tiered trailing
    if (peakProfitPct >= CONFIG.tier3TriggerPct) {
        trailDist = CONFIG.tier3TrailPct; newTier = 3;
    } else if (peakProfitPct >= CONFIG.tier2TriggerPct) {
        trailDist = CONFIG.tier2TrailPct; newTier = 2;
    }

    const trailingStop = hunt.peakPrice * (1 - trailDist);
    if (peakProfitPct >= CONFIG.tier2TriggerPct && trailingStop > stopPrice) {
        stopPrice = trailingStop;
    }

    // ─── EXIT 1: MOONSHOT TP (+20%) ───
    if (liveProfitPct >= CONFIG.moonshotPct) {
        return { shouldExit: true, reason: `🚀 Moon-Shot TP (+${(liveProfitPct * 100).toFixed(2)}%)`, exitPrice: livePrice };
    }

    // ─── EXIT 2: EMERGENCY HARD STOP ───
    if (livePrice <= stopPrice) {
        const reason = stopPrice > hunt.entryPrice
            ? `Trailing Stop (Break-Even Protected)`
            : `Hard Stop-Loss (-${(CONFIG.hardStopPct * 100).toFixed(0)}%)`;
        return { shouldExit: true, reason: `🚨 ${reason}`, exitPrice: livePrice };
    }

    // ─── EXIT 3: HYPER-SCALP (Quick profit reversal) ───
    const reversalDist = (hunt.peakPrice - livePrice) / hunt.peakPrice;
    if (peakProfitPct >= CONFIG.scalpTriggerPct && reversalDist >= CONFIG.scalpReversalPct) {
        return { shouldExit: true, reason: `📉 Hyper-Scalp (${(reversalDist * 100).toFixed(2)}% from peak)`, exitPrice: livePrice };
    }

    // ─── EXIT 4: AVWAP TREND GUARD ───
    try {
        const entryTs = new Date(hunt.entryTime).getTime();
        const avwapResult = await calculateAnchoredVwap(hunt.symbol, entryTs);

        if (avwapResult && avwapResult.candleCount >= 3) {
            const prevAVWAP = hunt.lastVwapAnchor || 0;
            if (prevAVWAP > 0 && avwapResult.avwap < prevAVWAP) {
                return { shouldExit: true, reason: `📉 AVWAP Slope Negative (${avwapResult.avwap.toFixed(4)} < ${prevAVWAP.toFixed(4)})`, exitPrice: livePrice };
            }
            hunt.lastVwapAnchor = avwapResult.avwap;
        }
    } catch (e) {
        log('Exit', '⚠️', `AVWAP Error ${hunt.symbol}: ${e.message}`);
    }

    // ─── EXIT 5: RSI EXHAUSTION ───
    try {
        const { rsi, last5mClose } = await fetchRSI5m(hunt.symbol);
        if (rsi >= CONFIG.rsiExhaustionLevel && livePrice < last5mClose) {
            return { shouldExit: true, reason: `🥵 RSI Exhaustion (RSI: ${rsi.toFixed(0)})`, exitPrice: livePrice };
        }
    } catch (e) {
        log('Exit', '⚠️', `RSI Error ${hunt.symbol}: ${e.message}`);
    }

    // ─── EXIT 6: CANDLE CLOSE STOP ───
    if (candleClose <= stopPrice) {
        const reason = stopPrice > hunt.entryPrice ? 'Take Profit/BE (15m Close)' : 'Stop-Loss (15m Close)';
        return { shouldExit: true, reason: `🔴 ${reason}`, exitPrice: candleClose };
    }
 ...
 * The basket is the collection of all active rotation positions
 *
 * Basket Exit Triggers:
 *   1. DYNAMIC TARGET:     Total PnL >= min(bestPnL × 0.8, 8.0%)
 *   2. STRONG RUNNER:      Best single token at +8%
 *   3. CAPITAL PROTECTION:  Total near breakeven AND worst at -2%+
 */
function checkBasketExit(rotationActive) {
    if (rotationActive.length === 0) return null;
    ...
    if (totalPnL >= dynamicTarget && totalPnL > 0) {
        return { reason: `Dynamic Basket Target (+${totalPnL.toFixed(2)}% vs ${dynamicTarget.toFixed(2)}% target)`, totalPnL, bestPnL, type: 'profit' };
    }
    if (bestPnL >= 8.0) {
        return { reason: `Strong Runner (Top at +${bestPnL.toFixed(2)}%)`, totalPnL, bestPnL, type: 'profit' };
    }
    if (Math.abs(totalPnL) <= 0.5 && worstPnL <= -2.0 && rotationActive.length >= 2) {
        return { reason: `Capital Protection (Worst: ${worstPnL.toFixed(2)}% at breakeven)`, totalPnL, bestPnL, type: 'protection' };
    }
...
function registerPosition(symbol, entryPrice, density, distance, entryRsi) {
    ...
    sendTelegram([
        `${isSqueeze ? '🔥' : '🛰️'} <b>VWAP ROTATION ENTRY: #${symbol}</b>`,
        ``,
...
async function runScanner() {
    ...
    try {
        ...
        const rotationActive = allHunts.filter(h => h.status === 'active' && h.strategyId === 'golden_rotation');
        ...
        // ─── STEP 2: Basket Exit Check ───
        const basketResult = checkBasketExit(rotationActive);
        if (basketResult) {
            log('Scanner', '🧺', `BASKET ${basketResult.type.toUpperCase()}: ${basketResult.reason}`);
            ...
            for (const h of hunts) {
                if (symbolsToClose.includes(h.symbol) && h.strategyId === 'golden_rotation' && h.status === 'active') {
                    ...
                    h.reason = basketResult.reason;
                    updateBalance(h.pnl, h.capital);
                }
            }
            saveHunts(hunts);

            await sendTelegram([
                `🧺 <b>BASKET ${basketResult.type === 'profit' ? 'PROFIT TAKEN' : 'PROTECTED'}</b>`,
                ``,
                `<b>Reason:</b> ${basketResult.reason}`,
                `<b>Total PnL:</b> ${basketResult.totalPnL >= 0 ? '+' : ''}${basketResult.totalPnL.toFixed(2)}%`,
                `<b>Best Token:</b> +${basketResult.bestPnL.toFixed(2)}%`,
        ...
        if (totalPotentialOpenings > 0 && candidates.length > 0) {
            for (const cand of candidates.slice(0, totalPotentialOpenings)) {
                ...
                const isAlreadyIn = latestHunts.some(h => h.symbol === cand.symbol && h.status === 'active' && h.strategyId === 'golden_rotation');
                ...
                if (currentOpenCount >= CONFIG.maxSlots) {
                    ...
                    if (swapTarget) {
                        ...
                        const hunts = loadHunts();
                        for (const h of hunts) {
                            if (h.symbol === swapTarget.symbol && h.strategyId === 'golden_rotation' && h.status === 'active') {
...
async function runTracker() {
    ...
    try {
        const hunts = loadHunts();
        const active = hunts.filter(h => h.status === 'active' && h.strategyId === 'golden_rotation');
...
async function boot() {
    console.log('');
    console.log('═══════════════════════════════════════════════════════════════════');
    console.log('  🛰️  VWAP ROTATION — STANDALONE AUTONOMOUS BOT');
    ...
    const config = loadConfig();
    const hunts = loadHunts();
    const activeHunts = hunts.filter(h => h.status === 'active' && h.strategyId === 'golden_rotation');
    ...
    log('Boot', '⚙️', `Config: SL=-${(CONFIG.hardStopPct * 100).toFixed(0)}% | Scalp=+${(CONFIG.scalpTriggerPct * 100).toFixed(1)}%→${(CONFIG.scalpReversalPct * 100).toFixed(1)}%rev | Moon=+${(CONFIG.moonshotPct * 100).toFixed(0)}% | RSI>${CONFIG.rsiExhaustionLevel}`);

    // Telegram boot notification
    await sendTelegram([
        `🛰️ <b>VWAP ROTATION BOT STARTED</b>`,
        ``,
        ...
        ``,
        `<b>Exit Rules:</b>`,
        `• Stop-Loss: -${(CONFIG.hardStopPct * 100).toFixed(0)}%`,
        `• Hyper-Scalp: +${(CONFIG.scalpTriggerPct * 100).toFixed(1)}% → ${(CONFIG.scalpReversalPct * 100).toFixed(1)}% reversal`,
        `• Moonshot TP: +${(CONFIG.moonshotPct * 100).toFixed(0)}%`,
        `• RSI Exhaustion: >${CONFIG.rsiExhaustionLevel}`,
...
process.on('SIGINT', async () => {
    ...
    await sendTelegram(`🛑 <b>VWAP ROTATION BOT STOPPED</b>\n\n<i>Manual shutdown.</i>`);
    process.exit(0);
});
